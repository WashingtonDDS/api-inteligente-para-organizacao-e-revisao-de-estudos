...
- **axios (^1.7.2)**: Cliente HTTP baseado em Promises para realizar requisições a APIs e serviços externos de forma simples e eficiente.
- **bcrypt (^5.1.1)**: Biblioteca para hashing e verificação de senhas, utilizada para proteger senhas de usuários.
- **dotenv (^16.4.5)**: Carrega variáveis de ambiente a partir de um arquivo `.env`, configurando informações sensíveis fora do código-fonte.
- **express (^4.19.2)**: Framework web para Node.js, facilitando a criação de servidores HTTP e o gerenciamento de rotas e middleware.
- **jsonwebtoken (^9.0.2)**: Biblioteca para criar e verificar JSON Web Tokens (JWTs), usada para autenticação e autorização.
- **pg (^8.12.0)**: Cliente PostgreSQL para Node.js, utilizado para conectar e interagir com bancos de dados PostgreSQL.
- **uuid (^9.0.1)**: Biblioteca para gerar identificadores únicos universais (UUIDs), útil para criar IDs únicos e seguros.

### DevDependencies

- **@jest/globals (^29.7.0)**: Tipagens globais para Jest, facilitando a utilização das funções globais de teste em projetos TypeScript.
- **@types/bcrypt (^5.0.2)**: Tipagens TypeScript para a biblioteca bcrypt.
- **@types/express (^4.17.21)**: Tipagens TypeScript para o framework express.
- **@types/jest (^29.5.12)**: Tipagens TypeScript para o Jest.
- **@types/jsonwebtoken (^9.0.6)**: Tipagens TypeScript para a biblioteca jsonwebtoken.
- **@types/pg (^8.11.6)**: Tipagens TypeScript para o cliente pg.
- **@types/supertest (^6.0.2)**: Tipagens TypeScript para supertest, usado para testar APIs HTTP.
- **@types/uuid (^10.0.0)**: Tipagens TypeScript para a biblioteca uuid.
- **jest (^29.7.0)**: Framework de testes para JavaScript, utilizado para escrever e executar testes automatizados.
- **jest-html-reporters (^3.1.7)**: Reporter para Jest que gera relatórios de teste em HTML.
- **nodemon (^3.1.0)**: Ferramenta que monitora alterações no código-fonte e reinicia automaticamente o servidor.
- **supertest (^7.0.0)**: Biblioteca para testar APIs HTTP.
- **ts-jest (^29.1.2)**: Transformador de Jest para TypeScript, permitindo a execução de testes escritos em TypeScript.
- **ts-node (^10.9.2)**: Ferramenta para executar arquivos TypeScript diretamente no Node.js.
- **typescript (^5.4.5)**: Superset de JavaScript que adiciona tipagem estática e outras funcionalidades ao JavaScript.

## Integração com a API do Gemini

A integração com a API do Gemini foi realizada para obter resumos automáticos com base em tópicos fornecidos. Aqui está um resumo de como a integração foi implementada:

### Passos para Integração

1. **Leitura da Documentação da API**: Compreensão dos endpoints disponíveis e parâmetros necessários.
2. **Configuração do axios**: Configuração para realizar requisições HTTP com headers, parâmetros de consulta e tratamento de respostas.
3. **Uso de Variáveis de Ambiente**: Armazenamento da chave da API em uma variável de ambiente (`process.env.GEMINI_API_KEY`) para segurança e flexibilidade.
4. **Tratamento de Respostas**: Extração e uso das informações relevantes das respostas da API.

## Uso do Docker e Docker Compose

Docker e Docker Compose foram utilizados para criar ambientes de desenvolvimento e produção consistentes e reproduzíveis.

### Docker

- **Objetivo**: Isolar a aplicação e suas dependências em containers.
- **Criação da Imagem Docker**: Arquivo `Dockerfile` para definir a imagem da aplicação.
- **Construção e Execução**: Comandos para construir e executar a imagem Docker.

### Docker Compose

- **Objetivo**: Facilitar a definição e o gerenciamento de múltiplos containers.
- **Arquivo `docker-compose.yml`**: Define serviços, redes e volumes necessários.
- **Execução**: Iniciar com `docker-compose up` e parar com `docker-compose down`.
